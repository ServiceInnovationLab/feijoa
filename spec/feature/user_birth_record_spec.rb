  ...
  describe '#query' do
    context 'A User' do
      ...
      context 'a target record and some other records exist' do
        ...
        it 'is found by an exact query' do
          click_on 'Find'
        ...
        it 'no records are found for a non-matching query' do
          fill_in 'birth_record_family_name', with: 'Not-Target-Person'
          click_on 'Find'

          expect(page).to_not have_css('.card')
          expect(page).to_not have_css("##{target_record.model_name.param_key}--#{target_record.primary_key_string}")
        end
      end
    end
  end
